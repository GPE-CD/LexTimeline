...
## Nota técnica da versão 1.2

A consulta de tramitações não usa mais `ordenarPor=dataHora`, pois esse parâmetro pode gerar erro HTTP 400 no endpoint `/proposicoes/{id}/tramitacoes` em algumas proposições. O aplicativo realiza chamadas mais simples ao endpoint e ordena os eventos localmente por data.


## Versão v1.4 sem pasta `data/`

Esta versão foi ajustada para facilitar o upload pelo GitHub Web. Os dados de demonstração estão embutidos no próprio `app.py`, de modo que não é necessário criar ou enviar a pasta `data/`.
...
No Streamlit Community Cloud, mantenha `app.py` como arquivo principal.
